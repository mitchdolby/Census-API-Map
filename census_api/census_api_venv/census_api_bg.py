...
def clean(df):
    df.columns = ['index', 'NAME', 'GEO_ID', 'INCOME', 'state', 'county', 'tract', 'block group']
    geoid = df['GEO_ID']
...
def main():
    ...
    if args.state.upper() not in states:
        ...
        exit(0)
    response = requests.get("https://api.census.gov/data/2019/acs/acs5?get=NAME,GEO_ID,B19013_001E&for=block group:*&in=state:{}&in=county:*&in=tract:*&key={}".format(states[args.state.upper()], api_key))
